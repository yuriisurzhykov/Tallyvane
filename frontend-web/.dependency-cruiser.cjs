...
module.exports = {
    forbidden: [
        ...
        {
            // `shared/ui/theme` (and the token engine it alone may import) moved to
            // the `frontend-shared` package as part of ADR-032, so the check for
            // this now lives in `packages/frontend-shared/.dependency-cruiser.cjs`
            // instead — this app no longer has any file that imports the engine.
