...
> Moved from `frontend-web/src/shared/ui/theme` into the `frontend-shared` package
> as part of ADR-032 (splitting `frontend-admin` into a genuinely separate
> app). Nothing below changed except where a path had to cross the new
> package boundary — see `packages/frontend-shared/README.md` for why.
...
semantic/     flat semantics with no theme axis: radius, spacing, border, typography, layout, control, z-index
