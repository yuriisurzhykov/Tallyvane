...
| Component                | Purpose                                                                                                                                                                                                                                                                                                           | Base    | Env    |
|--------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------|--------|
| `Button`                 | Tones: `primary` (monochrome solid fill), `neutral` (bordered, no fill), `ghost` (borderless, hover fill only), `danger` (solid `statusDanger` fill — reads as a real button, not a badge). Sizes via `control` height roles; loading state disables + `aria-busy` + inline spinner; leading/trailing icon slots. | Base UI | server |
...
| `Link`                   | Inline navigation text, and the `render` target when `Button` must be an anchor. One visual form (accent text + underline).                                                                                                                                                                                       | —       | server |
| `SkipLink`               | Visually hidden (`sr-only`) until focused (`focus:not-sr-only`), then jumps to the caller-given landmark id. Fixed, `z-toast`, no variants.                                                                                                                                                                       | —       | server |
| `Toggle` / `ToggleGroup` | Two-state and exclusive-choice buttons, pressed/value state fully delegated to Base UI (`data-pressed` styling).                                                                                                                                                                                                  | Base UI | client |
...
| `TextArea`                    | Auto-growing multi-line.                                                                                                                                                                                                                                     | —       | server |
...
| `Checkbox` / `CheckboxGroup`  | A tickable box for an independent choice, or several sharing one set of ticked values.                                                                                                                                                                       | Base UI | client |
| `Radio` / `RadioGroup`        | One option in a mutually exclusive set, coordinated via a shared selected value.                                                                                                                                                                             | Base UI | client |
| `Switch`                      | On/off settings, e.g. per-rule reminder enablement.                                                                                                                                                                                                          | Base UI | client |
| `RatingScale`                 | A 1–5 scale, used by four unrelated fields: interest, fit, interview confidence, question difficulty. Generic because the scale is generic; the label is not its business. Five dots, a single pick; the selected dot and every dot before it render filled. | —       | client |
| `SearchField`                 | Text input with clear affordance and debounce.                                                                                                                                                                                                               | Base UI | client |
| `FileDrop`                    | Drag-and-drop file selection for résumés and media — single file; the actual upload happens at a higher tier.                                                                                                                                                | —       | client |
...
| Component     | Purpose                                                                                                                                                                       | Base    | Env    |
|---------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------|--------|
| `Collapsible` | **The third depth level.** Row expansion is this component inside a table row; the trigger stays layout-agnostic since callers want both a table-row caret and a full header. | Base UI | client |
| `Accordion`   | Coordinates several disclosures into one FAQ-style group, so opening one can close the others. No roving arrow-key focus — dropped upstream following an ARIA APG update.     | Base UI | client |
| `Tabs`        | Switches which of several panels is visible, segmented-control style (the bordered track on `List`, the filled pill on `Indicator`) — visually distinct from `ToggleGroup`.   | Base UI | client |
...
| `Tag`        | Removable chip for a tech tag or skill — `chip` radius, distinct from `Badge`'s `pill`.                                                                                                                                          | —       | server |
...
| `Spinner`    | Only for genuinely slow work — PDF render, media processing. Ordinary writes are optimistic and show nothing. Sized `sm`/`md`/`lg` from the `control` height roles; `Button`'s own inline loading state now composes this.       | —       | server |
...
| Component               | Purpose                                                                                                                                                                                                                                                                                                                                            |
|-------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `DataTable`             | Dense, virtualized, keyboard-navigable compound (`Root`/`Header`/`Body`/`Row`/`Cell`) with a slot for an expanded row (`Collapsible` inside a row). Column definitions come from the caller via `DataTableColumnDef`, TanStack's own `ColumnDef` specialized to this component's fixed feature set. Backed by TanStack Table v9 and Virtual — headless, so no styling arrives with them (`ADR-031`'s own named gap). One fixed row height for now, not density-aware yet — `data-density` has nothing to switch until §5's density tokens exist. This is the single most load-bearing component in the product: the pipeline is a thousand rows at 16 ms a frame. See `shared/ui/data-table/README.md`. |
...
| `InlineEdit`            | Built — `shared/ui/inline-edit/`. Click to edit, 400 ms debounce, optimistic write, failure surfaced through a caller-supplied `onError(error, retry)` rather than an assumed `ToastRegion`. **The default write interaction of the product**, not a special case. See its own `README.md` for the rollback-semantics decision and the debounce engine (`shared/lib/use-debounced-autosave.ts`) behind it. |
...
|---------------------|---------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `AppShell`          | Console and admin                           | Built 2026-08-28. Sidebar plus main region and the `#main-content` skip-link target; the floating action button is not built yet.                                                          |
| `SidebarNav`        | `AppShell`                                  | Built 2026-08-28, text-only. Renders as a horizontal scrollable row below `lg`, a vertical column from `lg` up — the 64px icon-collapsed role stays reserved for the day `Icon`'s API is decided. |
| `TopBar`            | `AppShell`                                  | Built 2026-08-28: title plus a generic `actions` slot. Search, theme, density and account are not built yet — real call sites will supply them through `actions`.                          |
