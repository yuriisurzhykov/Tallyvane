...
internal fun noBannedSuffix(scope: KoScope): List<String> =
    scope
...
internal fun portNaming(scope: KoScope): List<String> =
    scope
...
internal fun noImplSuffix(scope: KoScope): List<String> =
    scope
        .classesAndInterfacesAndObjects(includeNested = true)
...
internal fun adapterNamedByMechanism(scope: KoScope): List<String> =
    scope
...
internal fun noFakeInMain(scope: KoScope): List<String> =
    scope
        .classesAndInterfacesAndObjects(includeNested = true)
...
internal fun usecaseIsImperative(scope: KoScope): List<String> =
    scope
        .applicationUseCases()
...
internal fun appHasNoLogic(scope: KoScope): List<String> =
    scope
...
internal fun noHardcodedProductName(scope: KoScope): List<String> =
    scope.files
        .withoutException("no-hardcoded-product-name")
        .filter { file ->
            val allowed = unixPath(file).contains("/strings/") || unixPath(file).contains("/config/")
            !allowed && Regex("""\b$PRODUCT_NAME\b""").containsMatchIn(file.text)
        }.map { it.where() }
