        ...
        "own layer is required only when the feature has that layer" {
            val allow =
                GraphCheck.Features.Allow(
                    ...
                    feature = Feature("jobs", listOf("domain"), emptyList()),
                )
        ...
        "any:contract expands to each read's contract module" {
            val allow =
                GraphCheck.Features.Allow(
                    ...
                    feature = Feature("briefing", listOf("application"), listOf("jobs", "contacts")),
                )
                        ...
                        ModulesYamlFake(
                            platforms =
                                listOf(
                                    ...
                                    Platform("persistence", listOf("platform:kernel")),
                                ),
                            layerAllow = mapOf("infrastructure" to listOf("platform:*")),
                        ),
                    feature = Feature("jobs", listOf("infrastructure"), emptyList()),
                )
