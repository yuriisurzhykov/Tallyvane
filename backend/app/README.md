...
`Configuration` or `Application` under `app/src/`, and `one-top-level-class` admits one type per file.
...
./gradlew :app:run         # listens on 8080 unless TALLYVANE_HTTP_PORT says otherwise
```
...
**The full schema-drift run.** It needs the app to find every `Table` object on the classpath, and how
to scan is an unmade decision — see `backend/.plans/`. With no capability module in existence the test
could only assert that a scan found nothing, which is a canary, not coverage.
