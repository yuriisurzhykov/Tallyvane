...
class ConfirmEmailMfaEnrollmentSpec : StringSpec({
    "confirmation enrolls only after consuming this user's MFA challenge once" {
        ...
        val challenges = EmailChallenges(store, object : EmailDelivery {
            override suspend fun sendCode(email: Email, purpose: EmailChallengePurpose, code: Secret) = Unit
        }, ConfirmCodes(), TransactionRunnerFake(), IdGeneratorFake(), ClockFake(NOW))
        val challenge = challenges.issue(email, EmailChallengePurpose.MFA,
...
private class ConfirmChallengeStore : EmailChallengeStore {
    ...
    override suspend fun consume(id: Uuid, hash: Secret, now: Instant): Boolean {
        ...
        if (challenge.expiresAt < now || expected != hash) return false
        records.remove(id)
        return true
    }
    override suspend fun revoke(id: Uuid) { records.remove(id) }
}
