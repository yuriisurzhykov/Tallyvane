...
class VerifyRegistrationEmailSpec :
    StringSpec({
        "only a valid registration-purpose challenge verifies the matching account email" {
            ...
            val service = EmailChallenges(
                ...
                object : EmailDelivery {
                    override suspend fun sendCode(
                        email: Email,
                        purpose: tallyvane.identity.domain.email.EmailChallengePurpose,
                        code: Secret,
                    ) = Unit
                },
                FixedCodes(),
            ...
            val challenge = service.issue(
                email,
                tallyvane.identity.domain.email.EmailChallengePurpose.REGISTRATION,
...
private class ChallengeStore : EmailChallengeStore {
    ...
    override suspend fun issue(
        ...
        hashes[challenge.id] = hash
        remaining[challenge.id] = maxAttempts
        return true
    }
    override suspend fun find(id: Uuid): EmailChallenge? = challenges[id]
    override suspend fun consume(id: Uuid, hash: Secret, now: Instant): Boolean = challenges[id]?.takeIf { challenge ->
        id !in consumed && challenge.expiresAt > now && (remaining[id] ?: 0) > 0
    }?.let {
        remaining[id] = remaining.getValue(id) - 1
        (hashes[id] == hash).also { matches -> if (matches) consumed += id }
    } ?: false
