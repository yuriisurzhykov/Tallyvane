...
class BackupSecondFactorSpec : StringSpec({
    "backup code is enrolled only while codes remain and each code can be used once" {
        ...
        method.verify(user, tallyvane.identity.application.port.SecondFactorProof("recovery-1")) shouldBe true
        method.verify(user, tallyvane.identity.application.port.SecondFactorProof("recovery-1")) shouldBe false
        method.isEnrolledFor(user) shouldBe true
        EnrollSecondFactorUseCase.Enroll(SecondFactorMethodRegistry.Default(listOf(method)), TransactionRunnerFake())
            .enroll(EnrollSecondFactorRequest(user, tallyvane.identity.domain.secondfactor.SecondFactorKind.BACKUP_CODE)) shouldBe null
    }
})
...
private class RecoveryStore : BackupCodeStore {
    ...
    override suspend fun consume(userId: UserId, hash: Secret): Boolean = stored[userId]?.remove(hash.revealed()) == true
