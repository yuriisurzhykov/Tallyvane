...
class ConfirmRequiredFactorEnrollmentSpec :
    StringSpec({
        fun sessionIssuer(now: Instant) = SessionIssuer.Default(
        ...
        "activates the required factor and consumes its pending challenge" {
            ...
            val outcome = useCase.confirm(
                ConfirmRequiredFactorEnrollmentUseCase.Request(id, SecondFactorKind.TOTP, "123456"),
            ).shouldBeInstanceOf<ConfirmRequiredFactorEnrollmentUseCase.Outcome.Issued>()
            outcome.session.session.userId shouldBe userId
            outcome.session.session.device shouldBe DeviceLabel("Browser")
        ...
        "does not allow MFA verification challenges to complete enrollment" {
            ...
            useCase.confirm(
                ConfirmRequiredFactorEnrollmentUseCase.Request(id, SecondFactorKind.TOTP, "123456"),
            ) shouldBe ConfirmRequiredFactorEnrollmentUseCase.Outcome.NotConfirmed
