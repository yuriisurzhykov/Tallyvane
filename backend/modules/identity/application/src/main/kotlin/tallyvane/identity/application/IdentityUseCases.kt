...
public class IdentityUseCases(
    ...
    private val issuer = SessionIssuer.Default(
        sessions, refreshTokens, tokens, hashes, clock, ids, accessTtl, refreshIdleTtl,
    )
    private val completer = AuthenticationCompleter.Default(registry, pending, issuer, ids, clock, pendingTtl)
    ...
    public val signInWithGoogleOAuth: SignInWithGoogleOAuthUseCase? = googleOAuthGateway?.let { gateway ->
        SignInWithGoogleOAuthUseCase.SignIn(
            gateway,
            GoogleSignInCompleter.Default(users, credentials, completer, transactions, ids, clock),
        )
    }
