...
class PasswordResetOverPostgresSpec : StringSpec({
    "a persisted reset-purpose challenge replaces the password once" {
        PostgresPersistence(PostgresFixture.migrated()).use { persistence ->
            ...
            persistence.transactions.inTransaction {
                ...
                credentials.save(userId, Credential.PasswordRecord(passwords.hash(Secret("original password phrase"))))
                Verdict.Commit(Unit)
            }
            ...
            val challenges = EmailChallenges(
                EmailChallengeStoreOverExposed(), delivery, codes, persistence.transactions,
                IdGenerator.Uuid7(), ClockFake(now), EmailChallengePolicy(),
            )
            val challenge = challenges.issue(email, EmailChallengePurpose.PASSWORD_RESET)!!
            val reset = ResetPasswordUseCase.Replace(users, credentials, passwords, challenges, persistence.transactions)

            reset.reset(ResetPasswordRequest(challenge.id, email, delivery.code, Secret("replacement password phrase"))) shouldBe true
            ...
            challenges.verify(challenge.id, email, EmailChallengePurpose.PASSWORD_RESET, delivery.code) shouldBe false
            persistence.transactions.inTransaction { Verdict.Commit(users.findById(userId)?.emailVerified) } shouldBe true
        }
    }
})
