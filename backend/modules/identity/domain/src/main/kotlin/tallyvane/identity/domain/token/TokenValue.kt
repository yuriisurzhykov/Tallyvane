 ...
 *
 * Never stored as-is: exists only between a `TokenFactory` minting it and a `TokenHasher` turning
 * it into a [HashedToken] for storage or comparison. The shape is validated here, at construction,
 * rather than trusted from whichever port handed the string over.
 *
 * The error message below names no part of the offending value: a malformed [TokenValue] is, in
 * the cases that matter, a real bearer credential that failed to parse, and reporting it verbatim
 * would put a live secret's fragments into a log or a stack trace (§17).
 */
@JvmInline
