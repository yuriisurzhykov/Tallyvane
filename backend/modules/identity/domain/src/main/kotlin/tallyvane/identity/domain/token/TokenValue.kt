 ...
 *
 * Never stored as-is: this is the value that exists only between a `TokenFactory` minting it and
 * a `TokenHasher` turning it into a [HashedToken] for storage or comparison. The shape is
 * validated here, at construction, rather than trusted from whichever port handed the string over
 * — the same idiom `platform:http`'s `BasePath` already uses for a value whose caller might get
 * it wrong.
 *
