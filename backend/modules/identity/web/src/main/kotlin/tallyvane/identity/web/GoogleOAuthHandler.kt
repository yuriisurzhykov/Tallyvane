...
internal class GoogleOAuthHandler(
    ...
    suspend fun start(call: io.ktor.server.application.ApplicationCall) {
        ...
        call.response.cookies.append(
            Cookie(COOKIE, "${challenge.state}.${challenge.verifier}",
    ...
    suspend fun callback(call: io.ktor.server.application.ApplicationCall) {
            ...
            call.response.cookies.append(
                Cookie(COOKIE, "", path = COOKIE_PATH, secure = secure, httpOnly = true,
            ...
            if (parts == null || parts.size != 2 || state == null || code == null || !constantTimeEquals(parts[0], state)) {
                call.respondRedirect("/auth/google?error=oauth_failed", permanent = false)
                return
            }
            when (val outcome = signIn.signIn(SignInWithGoogleOAuthRequest(code, parts[1], redirectUri, DeviceLabel("Browser")))) {
                ...
                is SignInOutcome.NotIssued -> {
                    when (val reason = outcome.reason) {
                        is tallyvane.identity.domain.outcome.AuthenticationOutcome.RequiresSecondFactor ->
                            call.respondRedirect("/mfa?pending_id=${reason.pendingId.value}&methods=${reason.availableMethods.joinToString(",") { it.name }}", permanent = false)
                        else -> call.respondRedirect("/auth/google?error=oauth_failed", permanent = false)
                    }
                }
            }
    }
