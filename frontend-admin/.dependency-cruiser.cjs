 ...
 * — see that file's header comment — scoped to this app's own `app/` and `src/`.
 */
module.exports = {
    forbidden: [
        {
        ...
        {
            // Mirrors `frontend-web/.dependency-cruiser.cjs`'s rule of the same name:
            // the theme source (and the token engine it alone may import) lives in
