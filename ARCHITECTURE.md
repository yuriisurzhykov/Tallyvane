...
- `frontend-shared`'s `shared/blocks` — контракт `BlockType`, `FieldDescriptor`, `BlockRegistry`, контекст и хук `useBlockRegistry()`;
- `content-kit`'s `widgets/hero-block`, `widgets/feature-grid-block`, … — по одному слайсу на тип блока, общему для обоих приложений (§12.5, ADR-032);
- каждого приложения `app/providers/block-registry` — собирает реестр из конкретных блоков и предоставляет его;
- `content-kit`'s `widgets/block-renderer` — рендерит по данным, обращаясь к реестру через хук, и **не импортирует ни одного конкретного блока**.
...
Список возможностей сознательно не расширен третьей записью вида «администратор платформы». ADR-005 уже заложил мультитенантность на уровне данных — колонка владельца и сквозной скоупинг существуют с первого дня, — но администрировать сегодня некого: пользователь один. `admin.tallyvane.com` (ADR-032) зарезервирован как поверхность, где такая возможность появится, когда появится второй арендатор, а не построен заранее ради гипотетического будущего — это был бы код без потребителя, который правило `no-utility-files` и дух YAGNI одинаково не одобряют.
...
Сессионная кука привязана ровно к тому хосту, что её выставил, а не к `Domain=.tallyvane.com`: с ADR-032 `app.tallyvane.com` и `admin.tallyvane.com` — разные приложения, и общая кука дала бы скомпрометированной сессии консоли путь в админку в обход и Cloudflare Access, и проверки `content.manage`. Вход на одном хосте не создаёт сессию на другом — владелец логинится дважды, и это осознанная цена за то, что кража одной сессии не равна краже обеих.
...
**Каталог `app/` в корне каждого приложения — не слой FSD, а адаптер маршрутизации.** Каждый файл маршрута состоит из одной строки:
...
Правило проверяется скриптом: файл маршрута не длиннее пяти строк, не содержит ничего кроме реэкспорта и метаданных, и в нём нет ни JSX, ни импортов из слоёв ниже `views`. Одновременно скрипт запрещает файлы с именами `page.tsx`, `layout.tsx`, `route.ts`, `template.tsx`, `loading.tsx`, `error.tsx` где-либо внутри `src/` — так исключается любая двусмысленность между FSD-слоем `src/app` и маршрутизацией Next. Правило применяется в обоих приложениях независимо.

**`frontend-shared` и `content-kit` — не слои FSD, а границы пакетов workspace**, подключаемые как `workspace:*` тем же механизмом, каким уже подключён `design-token-engine` (ADR-026). Внутри `frontend-shared` лежит ровно один слой — `shared`; внутри `content-kit` — два, `entities` и `widgets`, со своим направлением импорта, проверяемым отдельными конфигурациями `steiger`/`dependency-cruiser` каждого пакета. Ни то, ни другое не образует шестого слоя фронтенда: слои FSD по-прежнему `shared → entities → features → widgets → views → app`, просто нижние слои для обоих приложений частично живут за пределами их собственного `src/`.
...
**`packages/frontend-shared` (слой `shared`)** — сегменты `ui` (дизайн-система: скомпилированные токены, раскладка на Tailwind, поведение примитивов из Base UI — см. [ADR-031](#22-журнал-архитектурных-решений); полная опись компонентов по уровням в `packages/frontend-shared/src/shared/ui/COMPONENTS.md`), `api` (сгенерированный из OpenAPI клиент и обвязка запросов), `i18n` (движок строк и словари), `blocks` (контракт типа блока и реестра — сам реестр, не конкретные блоки), `lib` (узкие именованные модули), `config` (константы окружения). Используется обоими приложениями.

**`packages/content-kit` (слои `entities` и `widgets`)** — `entities/content-page`, `entities/media-asset`; `widgets/block-renderer` и по одному слайсу на тип контентного блока: `hero-block`, `feature-grid-block`, `faq-block`, `cta-block`, `screenshot-block`, `quote-block`. Используется обоими приложениями: `frontend-web` — для рендера опубликованных блоков, `frontend-admin` — для формы редактирования и живого превью тем же самым определением (ADR-013).

**`frontend-web` (`entities`, `features`, `widgets`, `views` консоли и публичных страниц)** — `entities`: `job`, `company`, `application`, `contact`, `communication`, `document`, `resume-bullet`, `interview`, `compensation`. `features`: `capture-job`, `apply-to-job`, `log-event`, `rate-job`, `schedule-interview`, `record-communication`, `send-connection-request`, `compose-resume`, `render-resume`, `evaluate-compensation`, `filter-pipeline`, `global-search`, `configure-reminders`. `widgets`: `pipeline-table`, `pipeline-board`, `today-actions`, `job-summary-card`, `brief-layout`, `contact-history`, `resume-studio`, `funnel-chart`, `activity-chart`. `views`: `landing`, `blog-index`, `blog-post`, `doc-page`, `changelog`, `legal-page`, `today`, `pipeline`, `job-detail`, `brief`, `contacts`, `contact-detail`, `resume-studio`, `analytics`, `settings`.

**`frontend-admin` (`features`, `widgets`, `views` — без локального `entities`)** — `features`: `edit-page-blocks`, `publish-page`, `restore-revision`, `upload-media`, `edit-strings`. `widgets`: `block-editor`. `views`: `admin-page-list`, `admin-page-editor`, `admin-media`, `admin-strings`. Локального слоя `entities` нет: единственные нужные ему сущности, `content-page` и `media-asset`, уже приходят из `content-kit`.
...
- Имя контейнеров, схем базы и переменных окружения нейтрально: `server`, `frontend-web`, `db`, а не производные от бренда.
...
Число модулей Gradle на потребление памяти в рантайме не влияет вовсе: в JVM попадает один набор классов независимо от того, из скольких модулей он собран. Симметрично, число приложений Next.js не влияет на Ktor: он остаётся одним процессом и не знает, что фронтендов стало два.

### 16.3. Маршрутизация и сеть

Правила туннеля разводят трафик по имени хоста, а внутри каждого хоста — по путям, как и раньше: `tallyvane.com` и `app.tallyvane.com` уходят в `frontend-web`, кроме `/api/*`, `/calendar/*` и `/media/*`, которые уходят в Ktor; `admin.tallyvane.com` — в `frontend-admin`, с тем же исключением для `/api/*` и `/media/*`, указывающим на тот же контейнер Ktor. Правило `/api/*` продублировано под обоими приватными хостами намеренно: это дешевле, чем заводить отдельный хост `api.*` и вместе с ним CORS, которого в системе нет нигде больше. Node не оказывается в горячем пути вызовов API ни для одного из двух фронтендов и не отдаёт файлы.

`admin.tallyvane.com` — единственный хост с дополнительным рубежом: запрос сперва проходит Cloudflare Access, который проверяет личность до того, как достигнет `cloudflared`. Это заменяет собой отдельную VPN-инфраструктуру для доступа к админке — рубеж навешивается на уже существующий туннель, а не добавляет вторую платформу в эксплуатацию (тот же принцип, которым руководствовался выбор VPS в ADR-011).
...
**ADR-011. Единое приложение Next.js на все три поверхности, размещённое на своём VPS.** Появление CMS сделало серверный рендер обязательным для публичной части; раз рантайм Node всё равно нужен, отдельное SPA рядом становится лишней сущностью. Размещение на своём VPS выбрано вместо рендера на границе сети, потому что инструмент знаком владельцу и не добавляет вторую платформу в эксплуатацию. Цена — 320 МБ памяти и урезанная до 384 МБ куча JVM. **Уточнено, не отменено, ADR-032**: рассуждение выше остаётся верным для пары «блог + консоль», которая и сейчас один процесс; админ-поверхность получила отдельное приложение, потому что ей понадобилось свойство, которого группа маршрутов внутри общего процесса выразить не может, — недостижимость извне, а не другой URL.
...
**ADR-032. Три поддомена, два приложения, два пакета workspace.** Уточняет ADR-011: `tallyvane.com` и `app.tallyvane.com` остаются одним процессом Next.js (`frontend-web`), потому что рассуждение ADR-011 для этой пары не изменилось. `admin.tallyvane.com` получает отдельное приложение (`frontend-admin`), не зависящее от `frontend-web` как workspace-пакет, потому что был явный запрос: разработчик должен быть физически не в состоянии импортировать код консоли в админку или наоборот, а проверка по `Host` внутри общего билда такой гарантии не даёт — скомпилированные обработчики маршрутов админки остаются в графе модулей веб-инстанса, просто не достижимы обычным путём. Общее между приложениями — ровно то, что уже требовал ADR-013: тема, api-клиент и словарь строк вынесены в пакет `frontend-shared`, а система блоков контента (`entities/content-page`, `entities/media-asset`, `block-renderer` и типы блоков) — в пакет `content-kit`, отдельно от `frontend-shared`, потому что несёт бизнес-смысл, которого дизайн-система нести не должна.
